...
def take_photo1():
    cam = cv2.VideoCapture(0)  # öffnet Kamera
...
def Distanz_Check(SpeichernsPfad):
    matrixpfad=str(SpeichernsPfad)+'FinalCalibratedPhoto.txt'
    matrixAusText =np.loadtxt(matrixpfad)
    #TextPfad=os.path.join(SpeichernsPfad,'FinalCalibratedPhoto.txt' )
    #matrixAusText=np.loadtxt(TextPfad)
    #print(matrixAusText)
    ...
    SizeMatrix_FinlMatrx = len(FinalMatrix[0])
    while SizeMatrix_FinlMatrx > 3:    											# Löschen aller Abständen
        FinalMatrix=np.delete(FinalMatrix,SizeMatrix_FinlMatrx-1,1)				# Löschen aller Abständen
        SizeMatrix_FinlMatrx -=1
    ...
    while ix < len(matrixAusText):
        ...
        jx=0
        ix +=1
    AusgangMatrix = np.delete(AusgangMatrix, np.where(AusgangMatrix==1.0)[0], axis=0)	# löschen die eingabe von Label 1
    AusgangMatrix = np.delete(AusgangMatrix, np.where(AusgangMatrix==0.0)[0], axis=0)	# löschen die eingabe von Label 0
   # SpeichernsPfad = str(SpeichernsPfad)+'AusgangMatrix.txt' 
   # np.savetxt(SpeichernsPfad,AusgangMatrix)
    return(AusgangMatrix)
